#include "Components/FINAnyBoxHolo.h"
#include "FGBuildableBeam.h"
#include "FGBuildablePillar.h"

AFINAnyBoxHolo::AFINAnyBoxHolo() {
	PrimaryActorTick.bCanEverTick = true;
	SetActorTickEnabled(true);
}

void AFINAnyBoxHolo::Tick(float DeltaSeconds) {
	Super::Tick(DeltaSeconds);
}

void AFINAnyBoxHolo::EndPlay(const EEndPlayReason::Type EndPlayReason) {
...
void AFINAnyBoxHolo::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const {
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
}

bool AFINAnyBoxHolo::DoMultiStepPlacement(bool isInputFromARelease) {
	return true;
}

int32 AFINAnyBoxHolo::GetBaseCostMultiplier() const {
	return 1;
}
...
//#pragma optimize("", off)
void AFINAnyBoxHolo::SetHologramLocationAndRotation(const FHitResult& HitResult) {
...
AActor* AFINAnyBoxHolo::Construct(TArray<AActor*>& out_children, FNetConstructionID netConstructionID) {
	return Super::Construct(out_children, netConstructionID);
}

void AFINAnyBoxHolo::CheckValidFloor() {}

void AFINAnyBoxHolo::ConfigureActor(AFGBuildable* inBuildable) const {
