#include "RFINNetworkCableHologram.h"
...
void ARFINNetworkCableHologram::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const {
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	DOREPLIFETIME(ARFINNetworkCableHologram, Snapped);
	DOREPLIFETIME(ARFINNetworkCableHologram, OldSnapped);
	DOREPLIFETIME(ARFINNetworkCableHologram, From);
}

bool ARFINNetworkCableHologram::DoMultiStepPlacement(bool isInputFromARelease) {
...
UFINNetworkConnectionComponent* ARFINNetworkCableHologram::SetupSnapped(FFINCablePlacementStepInfo s) {
	switch (s.SnapType) {
	case FIN_CONNECTOR:
...
AActor* ARFINNetworkCableHologram::Construct(TArray<AActor*>& childs, FNetConstructionID constructionID) {
	...
	if (Snapped.SnapType == EFINNetworkCableHologramPlacementStepType::FIN_POLE) {
...
void ARFINNetworkCableHologram::ConfigureActor(AFGBuildable* inBuildable) const {
...
int32 ARFINNetworkCableHologram::GetBaseCostMultiplier() const {
...
bool ARFINNetworkCableHologram::IsValidHitResult(const FHitResult& hit) const {
	return hit.Actor.IsValid();
}

bool ARFINNetworkCableHologram::TrySnapToActor(const FHitResult& hitResult) {
...
bool ARFINNetworkCableHologram::IsSnappedValid() {
	bool ret = true;
	if (Snapped.SnapType == FIN_NOT_SNAPPED) {
...
void ARFINNetworkCableHologram::SetHologramLocationAndRotation(const FHitResult& hit) {
...
bool ARFINNetworkCableHologram::IsChanged() const {
	return Snapped.SnapType != FIN_NOT_SNAPPED;
}

USceneComponent* ARFINNetworkCableHologram::SetupComponent(USceneComponent* attachParent, UActorComponent* templateComponent, const FName& componentName) {
	return nullptr;
}

void ARFINNetworkCableHologram::SpawnChildren(AActor* hologramOwner, FVector spawnLocation, APawn* hologramInstigator) {
...
void ARFINNetworkCableHologram::OnInvalidHitResult() {
	OnEndSnap(Snapped);
	OnEndSnap(OldSnapped);
	Snapped.SnapType = OldSnapped.SnapType = FIN_NOT_SNAPPED;
}

void ARFINNetworkCableHologram::UpdateSnapped() {
...
void ARFINNetworkCableHologram::OnBeginSnap(FFINCablePlacementStepInfo a, bool isValid) {
...
void ARFINNetworkCableHologram::OnEndSnap(FFINCablePlacementStepInfo a) {
	if (a.SnapType != FIN_NOT_SNAPPED) {
		AActor* o = a.GetActor();
...
ARFINNetworkCableHologram::ARFINNetworkCableHologram() {
	UStaticMesh* cableMesh = LoadObject<UStaticMesh>(NULL, TEXT("/Game/RozeFINAddons/Components/ModularSystem/ThinNetworkCable/Mesh_ThinNetworkCable.Mesh_ThinNetworkCable"));
