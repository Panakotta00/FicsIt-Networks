...
BeginFunc(get, "Get", "Allows to get a single pixel from the buffer at the given position", 2) {
	...
	Body()
	const FFIRGPUT1BufferPixel& Pixel = self->Get(x, y);
...
BeginFunc(set, "Set", "Allows to set a single pixel of the buffer at the given position", 2) {
	...
	Body()
	if (c.Len() < 1) return;
	done = self->Set(x, y, FFIRGPUT1BufferPixel(c[0], foreground, background));
} EndFunc()
BeginFunc(copy, "Copy", "Copies the given buffer at the given offset of the upper left corner into this buffer.", 2) {
	...
	InVal(2, RStruct<FFIRGPUT1Buffer>, buffer, "Buffer", "The buffer from wich you want to copy from")
	...
	Body()
	self->Copy(x, y, buffer, (EFIRGPUT1TextBlendingMethod)textBlendMode, (EFIRGPUT1ColorBlendingMethod)foregroundBlendMode, (EFIRGPUT1ColorBlendingMethod)backgroundBlendMode);
} EndFunc()
...
BeginFunc(fill, "Fill", "Draws the given character at all given positions in the given rectangle on-to the hidden screen buffer.", 2) {
	...
	self->Fill(x, y, width, height, FFIRGPUT1BufferPixel(character[0], foreground, background));
} EndFunc()
BeginFunc(setRaw, "Set Raw", "Allows to set the internal data of the buffer more directly.", 2) {
	...
	if (foreground.Num() != background.Num()) success = false;
		...
		ParallelFor(Length, [self, Width, &characters, &foreground, &background](int i) {
			...
			self->Set(i % Width, i / Width, FFIRGPUT1BufferPixel(characters[i], ForegroundColor, BackgroundColor));
		});
		success = true;
	}
} EndFunc()
BeginFunc(clone, "Clone", "Clones this buffer into a new struct") {
	OutVal(0, RStruct<FFIRGPUT1Buffer>, buffer, "Buffer", "The clone of this buffer")
	Body()
	buffer = (FIRStruct) *self;
} EndFunc()
EndStruct()

BeginStruct(FFIRLogEntry, "LogEntry", "Log Entry", "An entry in the Computer Log.")
...
BeginStructConstructable(FFIRGPUT2DC_Box, "GPUT2DrawCallBox", "GPU T2 Box Draw Call", "This struct contains the necessary information to draw a box onto the GPU T2.")
