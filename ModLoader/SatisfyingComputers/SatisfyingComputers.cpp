...
SML::Mod::Mod::Info modInfo {
	...
	MOD_NAME,
	"1.0.0",
	"This Mod adds Computertechnology to the game.",
...
SDK::USceneComponent* FGBuildable_SetupComponent(SDK::AFGBuildable* self, SDK::USceneComponent* attachParent, SDK::UActorComponent* componentTemplate, const FName& componentName) {
	...
	if (!mesh) mesh = (SDK::UStaticMesh*)Functions::loadObjectFromPak(SDK::UStaticMesh::StaticClass(), L"/Game/FactoryGame/SatisfyingComputers/ComputerNetwork/Mesh_NetworkConnectorHolo.Mesh_NetworkConnectorHolo");
...
void FGBuildable_Dismantle(SDK::AFGBuildable* self_i) {
	...
	if (!cable) cable = (SDK::UClass*)Functions::loadObjectFromPak(L"/Game/FactoryGame/SatisfyingComputers/ComputerNetwork/NetworkCable/NetworkCable.NetworkCable_C");
...
class SatisfyingComputers : public Mod {
private:
	void beginPlay(Functions::ModReturns* modReturns, AFGPlayerController* playerIn) {
		const std::wstring bpPath = L"/Game/FactoryGame/SatisfyingComputers/ComputerController.ComputerController_C";
...
public:
	SatisfyingComputers() : Mod(modInfo) {
	}
	...
	void setup() override {
		...
		::subscribe<&AFGPlayerController::BeginPlay>(std::bind(&SatisfyingComputers::beginPlay, this, _1, _2));
		::subscribe<&Objects::AFGGameMode::InitGameState>([](Functions::ModReturns* ret, Objects::AFGGameMode* player) {
			Functions::loadObjectFromPak(L"/Game/FactoryGame/SatisfyingComputers/ComputerNetwork/NetworkManager/ED_NetworkMng.ED_NetworkMng");
		});
		...
		LOG("Finished SatisfyingComputers setup!");
	}
	...
	~SatisfyingComputers() {
		LOG("SatisfyingComputers finished cleanup!");
	}
};

MOD_API SML::Mod::Mod* ModCreate() {
	return new SatisfyingComputers();
}