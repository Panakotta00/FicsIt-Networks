...
executed per factory tick. While such a tick occurs it goes through two tick states.
...
the tick to end when an API call (not standard Lua function) occurs.
But when the tick doesn't end until a specific amount of code increments occur,
the system will crash with an out of time exception.
That means, when you make an infinite loop without any api calls.
The system will crash, but if you do an API call like print(), it will run as intended.
