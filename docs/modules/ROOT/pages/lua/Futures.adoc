...
When you call a function that will take some time to be processed you will get a Future as return value instead of having your Lua computer being blocked until the value becomes available.
...
It's also important to mention that the `await()` call will fail if the future itself errors or gets canceled.
[source,Lua]
----
local f = event.waitFor{}
local e, s, ... = f:await()
...
This entire system is inspired by Rusts Futures, its async/await and JavaScript's Event-Loop.
...
[WARNING]
====
It is important to note, that depending on the Future, you may have to `await()` (or repeatedly `poll()`) a future for it to actually do something.
Some others may work just fine, and others may be "canceled".
As rule-of-thumb, dormant futures that exist but never get polled/awaited are a waste of memory and maybe CPU time.
====

== Creating Futures

It can be useful to create your own futures from functions/coroutines if you want to have multiple separate sequences waiting for futures independently.
...
The values passed after the function, will be passed as parameters to the function itself.
...
Your function will have all the time it wants, but to make actually use of the future you just created, you have to use its functionality.
...
This can be relevant to some futures, and they will do some special processing when they get "canceled".
...
When the cancellation of a future occurs, the coroutine itself will be closed.
...
The global `defer(function)` allows you to execute a function when the return value closes.
...
You can also use the `future.loop()` function to indefinitely execute `future.run()`.
...
This behavior is reflected in a futures interface.

The `poll()` member function will execute the actual behavior (like resuming a coroutine or checking if the HTTP response finally got received).
It returns true when the polling has completed, the result values have been stored internally in the future and no further calls are necessary.
...
In the case the "poll behavior" (like the wrapped coroutine) causes an error, poll will simply forward this error down.
...
You can call this function as often as you want, but calling it on an unresolved or canceled future will cause an error.
