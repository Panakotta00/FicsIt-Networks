...
A value of such a type is also known as `Instance` or `Reference` to an object.
...
Tho read-only properties will cause an error when you try to assign a value to them
...
To work around this, we introduce the Futures.
