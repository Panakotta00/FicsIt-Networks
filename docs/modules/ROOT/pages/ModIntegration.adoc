...
Generally a reflection system is use in usually a programming language to allow the code base dynamically adapt and analyze its own and additional structure and functionality.
With it, you can f.e. figure out which kind of classes you can use, what functions there are and how to call them,
and sometimes you even get a metadata like documentation in terms of descriptions about those.
...
It additionally adds an abstract interface to make interacting with the data way more easily,
especially since the underlying data sources differ heavily. +
It also contains a lot of metadata so you can give localized names and descriptions to all the different things.
...
One of the most important things is, it only contains data about functions and attributes the user should actually be able to interact with by them self,
so that you can't f.e. change the position of an actor which would be the case if I just expose unreal's system. +
It provides a central interface for interaction with any functionality the game has with which a user can interact,
...
because FIN provides an `Any` struct, which can hold any kind of value supported by the reflection system.
And if your last parameter of a function is of type `TArray<FFINAnyNetworkValue>` and called `vararg`,
...
The actor itself can then have additional functions and properties with special names.
...
* "Any" (placeholder for any kind of supported data type, especially useful with arrays)
...
f.e. create a string variable `netProp_myString` and the class/blueprint will have now a `myString` variable, and you can use the
class variable to get/set the value.

WARNING: Replicated variables are not allowed and MIGHT cause crashes, it doesn't have to be directly visible to you that it doesn't work.
...
You can now optionally add a `netPropSet_` + property name, this will make the property now also writeable and this function will be called when you try to set the value.
...
The reflection system will detect it, and effectively replace its function body with code by FIN that will then trigger the corresponding
signal when you now simply call this function in your own code anywhere. +
...
f.e. `netFuncMeta_myFunc`, that takes no input values and can provide specific output parameters
(you can decide which ones you want and also in which order, the system doesn't care).
...
* A FString with the name "InternalName" which is basically an override for the usual function name which is used the in Lua etc. so with that you can make the function netFunc_BloodyHellFunction to be called completely differently like niceFunction instead of BloodyHellFunction
...
* A FText with the localized description of the function called "Description", should be self-explanatory
...
* A TArray<FText> called "ParameterDescriptions" same as "Description" just for each property of the netFunc_ (order is important as you can imagine)
* An integer called `Runtime` that can be 0 = Synchronous, 1 = Parallel or 2 = Asynchronous used to define in which runtime state the function can run directly.
...
but you have just the InternalName, DisplayName and Description which apply to the class itself,
...
It's still usefully especially for non FGBuildable classes ^^

NOTE: Those functions can be implemented also in BP but be aware that they are called onto the default object.

https://blueprintue.com/render/x3vj1r39/[This example] shows the metadata function `netFuncMeta_getModule` of the `getModule` function we have defined previously: +
++++
...
++++
// scrolling="no" Obsolete attribute
