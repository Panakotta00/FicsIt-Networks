...
If not empty, should be an image reference to the image that should be drawn inside the rectangle.
...
The size of the internal image drawn, necessary for proper scaling, antialiasing and tiling.
...
If true, the margin values provide a way to specify a fixed sized border thicknesses the boxes images will use( the image as 9-patch).
