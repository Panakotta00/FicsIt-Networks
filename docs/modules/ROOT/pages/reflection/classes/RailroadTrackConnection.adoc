...
This is an actor component for railroad tracks that allows to connecto to other track connections and so to connection multiple tracks with each eather so you can build a train network.
...
The world location of the connection.
...
! *Index* `index`
! Int
! The connection index to which the switch should be force to point to. Negative number to remove the lock.
!===
...
! *Index* `index`
! Int
! The index of the connection the switch currently points to.
!===
...
! *Track* `track`
! Trace<xref:/reflection/classes/RailroadTrack.adoc[RailroadTrack]>
! The track that the track pos points to.
