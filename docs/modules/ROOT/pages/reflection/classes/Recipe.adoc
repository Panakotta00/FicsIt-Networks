...
Returns an array of item amounts, this recipe needs (input) so the recipe can be processed.
...
Returns an array of item amounts, this recipe returns (outputs) when the recipe is processed once.
