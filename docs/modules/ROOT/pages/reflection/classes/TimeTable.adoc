...
Contains the timetable information of train.
...
The current number of stops in the timetable.
...
! *Added* `added`
! Bool
! True if the stop got successfully added to the timetable.
!===
...
! The timetable stop at the given index.
!===
...
Returns a list of all the stops this timetable has
...
! A list of timetable stops this timetable has.
!===
...
Sets the current stop to the next stop in the timetable.
...
Removes the stop with the given index from the timetable.
...
Allows to override a stop already in the timetable.
...
! *Stop* `stop`
! Struct<xref:/reflection/structs/TimeTableStop.adoc[TimeTableStop]>
! The timetable stop you want to override with.
!===
...
Allows to empty and fill the stops of this timetable with the given list of new stops.
