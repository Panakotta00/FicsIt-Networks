...
===== *Transfer Item* `transferItem` (_Int_ *Input* `input`, _Bool_ *Transferred* `transferred` _out_)
...
===== *Transfer Item* `transferItem` (_Int_ *Output* `output`, _Bool_ *Transferred* `transferred` _out_)
...
The currently inflow rate of fluid.
...
Sends a network message to all components in the network message network (including networks separated by network routers) on the given port. The data you want to add can be passed as additional parameters. Max amount of such parameters is 7 and they can only be nil, booleans, numbers and strings.
...
The Lever Module for the modular I/O Panel is used to switch a program state between two different value (on or off).
...
This large modular I/O control panel allows you to attach multiple different modules on to it and use them as I/O to control you programs.
 +
...
This large vertical modular I/O control panel allows you to attach multiple different modules on to it and use them as I/O to control you programs.
 +
...
===== *Get Closest Track Position* `getClosestTrackPosition` (_Struct(xref::Reflection.adoc#Struct-Vector[Vector])_ *World Pos* `worldPos`, _Trace(xref::Reflection.adoc#Class-RailroadTrack[RailroadTrack])_ *Track* `track` _out_, _Float_ *Offset* `offset` _out_, _Float_ *Forward* `forward` _out_)
...
Sets the prefab sign data e.g. the user settings like colo and more to define the signs content.
...
Returns the prefab sign data e.g. the user settings like colo and more to define the signs content.
...
===== *Self Driving Update* `SelfDrivingUpdate` (_Bool_ *Enabled* `enabled`)
...
===== _Float_ *Gravitational Force* `gravitationalForce`
...
===== _Float_ *Resistive Force* `resistiveForce`
...
[#Class-RailroadVehicleMovement-Funcs-getCouplerRotationAndExtension]
// tag::Class-RailroadVehicleMovement-Funcs-getCouplerRotationAndExtention__title[]
===== *Get Coupler Rotation And Extension* `getCouplerRotationAndExtension` (_Int_ *Coupler* `coupler`, _Float_ *X* `x` _out_, _Float_ *Y* `y` _out_, _Float_ *Z* `z` _out_, _Float_ *Extension* `extension` _out_)
...
| *Buffer* `buffer`
| Struct(xref::Reflection.adoc#Struct-GPUT1Buffer[GPUT1Buffer])
| The buffer from which you want to copy from
...
===== _Struct(xref::Reflection.adoc#Struct-Color[Color])_ *bBackground* `background`
...
The rule set which describe when the train will depart from the train station
...
Contains information about the rules that describe when a trian should depart from a station
