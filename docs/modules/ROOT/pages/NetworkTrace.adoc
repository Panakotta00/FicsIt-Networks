...
Network Traces are an important system of FicsIt-Networks which allows us to validate the availability
...
But, what happens now is, if you remove the connection of the computer network circuit to the power generator, then you should not be able to access the power circuit anymore.
But with a direct reference you are able to do so.

== Solution

What we now do is, we don't store just the reference to the object, but all references we used to get to the reference we actually want.
...
We use it nearly everywhere were we need to store a reference to another component like:
- Factory Hook
- Signal Listeners
